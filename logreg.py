...
def calculateNegativeLogLikelihood(X, y, w):
    A = y.T * np.log(1 / (1 + np.exp(-w.T @ X.T)) + epsilon)
    A += (1 - y.T) * np.log(1 - (1 / (1 + np.exp(-w.T @ X.T))) + epsilon)
    A = np.sum(A)
    return A

def gradient(X, y, w):
    A = ((1 / (1 + np.exp(-w.T @ X.T))).T - y).T @ X
    A = np.sum(A, axis=0)
...
def trainLogistic(X, y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        w_grad = gradient(X, y, w)#np.zeros((X.shape[1], 1))
